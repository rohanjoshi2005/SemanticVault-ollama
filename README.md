...
A vector database engine built from scratch in Python with a web UI.  
...
1. **Python 3.11+**
2. **Ollama** (runs the local AI models) — https://ollama.com
...
| Algorithms | HNSW, KD-Tree, Brute Force (built from scratch) |
...
| Port 8080 already in use | Edit last line of `main.py`: `uvicorn.run(app, port=9090)` |
| LLM answer is slow | Normal — llama3.2 takes 10–30s on CPU |
